...
class PDController:
    ...
    def __init__(
        ...
        self.dx_history = deque() #maxlen=3)
    ...
    def track(self, x):
        ...
        while should_continue:
            ...
            # Filtering velocity
            '''if(len(self.dx_history) == 3):
                dx_cur = -0.5*self.dx_history[-3] + 2*self.dx_history[-2] - 1.5*self.dx_history[-1]
                print(self.dx_history[-3])'''
            ...
            # Converting from operational space
...
if __name__ == "__main__":
    ...
    K_P = np.array([1e-5, 2.5e-6, 7.5e-3, 1e-2]) #np.array([0.004, 0.005, -0.0085, -0.000])
    K_I = np.array([-9e-6, -9e-6, -5e-3, -1e-3]) #np.array([0.0003, 0.005, -0.005, -0.000])
    K_D = np.array([1.25e-8, 7e-8, 2e-6, 1e-5]) #np.array([0.0000035, 0.000012, 0.0031, 0.000000])
    ...
    if len(c.motors) == 1:
        ...
        for i in range(len(c.motors)):
            ...
            axs2[i].axhline(cut_via[i], linestyle='--', color='green')
            axs2[i].axhline(move_via[i], linestyle='--', color='red')
            axs2[i].axhline(move_start[i], linestyle='--', color='blue')
            axs2[i].axhline(move_end[i], linestyle='--', color='purple')
        plt.xlabel("Time (s)")

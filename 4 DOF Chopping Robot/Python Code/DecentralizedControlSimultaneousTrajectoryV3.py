...
if __name__ == "__main__":
    ...
    K_P = np.array([260e-7, 180e-7, 200e-5, 60e-4]) #np.array([5e-5, 19e-6, 29e-4, 3e-2])
    K_I = np.array([90e-8, 100e-8, 180e-5, 55e-4]) #np.array([9e-6, 90e-7, 40e-4, 55e-3])
    K_D = np.array([40e-10, 190e-10, 180e-7, 16e-5]) #np.array([4e-8, 21e-9, 29e-7, 16e-4])
    ...
    n = 20
    # Creating trajectory
    t1, x1, dx1, ddx1 = getTraj(0, 5, 5*n, home, home_via)
    t2, x2, dx2, ddx2 = getTraj(0, 2, 2*n, home_via, cut1_via)
    t3, x3, dx3, ddx3 = getTraj_cutting(0, 3, 0.75, 3*n, cut1_via, cut1)
    t4, x4, dx4, ddx4 = getTraj(0, 2, 2*n, cut1, cut1_post)
    t5, x5, dx5, ddx5 = getTraj(0, 2, 2*n, cut1_post, cut2_via)
    t6, x6, dx6, ddx6 = getTraj_cutting(0, 3, 0.75, 3*n, cut2_via, cut2)
    t7, x7, dx7, ddx7 = getTraj(0, 2, 2*n, cut2, cut2_post)
    t8, x8, dx8, ddx8 = getTraj(0, 2, 2*n, cut2_post, cut3_via)
    t9, x9, dx9, ddx9 = getTraj_cutting(0, 3, 0.75, 3*n, cut3_via, cut3)
    t10, x10, dx10, ddx10 = getTraj(0, 2, 2*n, cut3, cut3_post)
    t11, x11, dx11, ddx11 = getTraj(0, 2, 2*n, cut3_post, move_via)
    t12, x12, dx12, ddx12 = getTraj(0, 2, 2*n, move_via, move_start)
    t13, x13, dx13, ddx13 = getTraj(0, 5, 5*n, move_start, move_end)
    t14, x14, dx14, ddx14 = getTraj(0, 5, 5*n, move_end, home_via)
    t15, x15, dx15, ddx15 = getTraj(0, 5, 5*n, home_via, home)

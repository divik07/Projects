...
if __name__ == "__main__":
    ...
    K_P = np.array([5e-5, 19e-6, 27e-4, 3e-2]) #np.array([5e-5, 1.5e-5, 2e-3, 3e-2])
    K_I = np.array([9e-6, 90e-7, 50e-4, 55e-3]) #np.array([9e-6, 9e-6, 5e-3, 5e-2])
    K_D = np.array([4e-8, 21e-9, 27e-7, 16e-4]) #np.array([4e-8, 1e-8, 2e-6, 9e-4])
    ...
    cut_via = [-150, 15, 270, 20] #[-150, 100, 270, 40]
    cut = [-150, 60, 270, -10] #[-150, 53, 270, -10]
    cut_via_2 = [-150, 80, 270, 30]
    move_via = [-100, 80, 270, 30]
    move_start = [-100, 53, 270, -5]
    move_end = [-200, 53, 270, -5]

    # Creating trajectory
    t1, x1, dx1, ddx1 = getTraj(0, 5, 80, home, home_via)
    t2, x2, dx2, ddx2 = getTraj(0, 5, 80, home_via, cut_via)
    t3, x3, dx3, ddx3 = getTraj_cutting(0, 3, 0.75, 48, cut_via, cut) #getTraj(0, 1, 16, cut_via, cut)
    #t3a, x3a, dx3a, ddx3a = getTraj(0, 2, 32, cut, cut)
    t4, x4, dx4, ddx4 = getTraj(0, 3, 48, cut, cut_via_2) #getTraj(0, 1, 16, cut, cut_via)
    t5, x5, dx5, ddx5 = getTraj(0, 3, 48, cut_via_2, move_via)
    t6, x6, dx6, ddx6 = getTraj(0, 2, 32, move_via, move_start)
    t7, x7, dx7, ddx7 = getTraj(0, 5, 80, move_start, move_end)
    ...
    # Defining intermediate positions
    home = [-50, 185, 215, 40]
    home_via = [-150, 15, 270, 40]
    cut1_via = [-175, 15, 270, 20]
    cut1 = [-175, 60, 270, -10]
    cut1_post = [-150, 80, 270, 30]
    cut2_via = [-150, 15, 270, 20]
    cut2 = [-150, 60, 270, -10]
    cut2_post = [-150, 80, 270, 30]
    cut3_via = [-125, 15, 270, 20]
    cut3 = [-125, 60, 270, -10]
    cut3_post = [-125, 80, 270, 30]
    move_via = [-100, 80, 270, 30]
    move_start = [-100, 53, 270, -5]
    move_end = [-200, 53, 270, -5]
    ...
    # Creating trajectory
    t1, x1, dx1, ddx1 = getTraj(0, 5, 80, home, home_via)
    t2, x2, dx2, ddx2 = getTraj(0, 2, 32, home_via, cut1_via)
    t3, x3, dx3, ddx3 = getTraj_cutting(0, 3, 0.75, 48, cut1_via, cut1)
    t4, x4, dx4, ddx4 = getTraj(0, 2, 32, cut1, cut1_post)
    t5, x5, dx5, ddx5 = getTraj(0, 2, 32, cut1_post, cut2_via)
    t6, x6, dx6, ddx6 = getTraj_cutting(0, 3, 0.75, 48, cut2_via, cut2)
    t7, x7, dx7, ddx7 = getTraj(0, 2, 32, cut2, cut2_post)
    t8, x8, dx8, ddx8 = getTraj(0, 2, 32, cut2_post, cut3_via)
    t9, x9, dx9, ddx9 = getTraj_cutting(0, 3, 0.75, 48, cut3_via, cut3)
    t10, x10, dx10, ddx10 = getTraj(0, 2, 32, cut3, cut3_post)
    t11, x11, dx11, ddx11 = getTraj(0, 2, 32, cut3_post, move_via)
    t12, x12, dx12, ddx12 = getTraj(0, 2, 32, move_via, move_start)
    t13, x13, dx13, ddx13 = getTraj(0, 5, 80, move_start, move_end)
    t14, x14, dx14, ddx14 = getTraj(0, 5, 80, move_end, home_via)
    t15, x15, dx15, ddx15 = getTraj(0, 5, 80, home_via, home)
